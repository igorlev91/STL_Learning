#include "xrstl/unique_ptr.h"
#include "xrstl/intrusive_ptr.h"

#include <memory>
#include <stdio.h>
#include <vector>

class MyRefCountObject : public xrstl::intrusive_ptr_interface_delete
{
public:
...
std::vector<xrstl::unique_ptr<MyObject>> crUniqueVector;

void RunUnitTestsSmartPtr()
{
	xrstl::intrusive_ptr<MyRefCountObject> objectHandle = new MyRefCountObject();
	...
	xrstl::unique_ptr<MyObject> crUniquePtr = xrstl::unique_ptr<MyObject>(new MyObject());
	crUniquePtr->PrintName();
	crUniquePtr = nullptr;

	stdUniqueVector.push_back(std::unique_ptr<MyObject>(new MyObject()));
	crUniqueVector.push_back(xrstl::unique_ptr<MyObject>(new MyObject()));
	...
	xrstl::unique_ptr<MyObject> crNullUniquePtr = nullptr;
	crNullUniquePtr = xrstl::unique_ptr<MyObject>(new MyObject());
