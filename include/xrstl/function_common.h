...
namespace xrstl
{
	...
	union functor_storage
	{
		...
		void* data() xrstl_noexcept { return &m_data[0]; }
		const void* data() const xrstl_noexcept { return &m_data[0]; }

		template<typename T>
		T& data() xrstl_noexcept { return *static_cast<T*>(data()); }

		template<typename T>
		const T& data() const xrstl_noexcept { return *static_cast<const T*>(data()); }
	...
	public:
		...
		static void create(functor_storage<Size>& destination, Fn&& fn)
		{
			...
			xrstl_constexpr_if(local)
			{
				::new (destination.data()) Functor(xrstl::forward<Fn>(fn));
			}
			else
			{
				destination.template data<Functor*>() = new Functor(xrstl::forward<Fn>(fn));
			}
		}

		static void destroy(functor_storage<Size>& destination)
		{
			xrstl_constexpr_if(local) // If local, just call destructor 
			{
		...
		template<typename Fn>
		static void init_functor(functor_storage<Size>& functor, Fn&& f) xrstl_noexcept
		{
			create(functor, xrstl::forward<Fn>(f));
		}

		static Result invoke(const functor_storage<Size>& functor, Args&&... args)
		{
			return (*get_pointer(functor))(xrstl::forward<Args>(args)...);
		}
