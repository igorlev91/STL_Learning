...
namespace xrstl
{
	...
	public:
		...
		fixed_function() xrstl_noexcept : m_invoker(nullptr), m_manager(nullptr) {}

		template<typename Functor>
		fixed_function(Functor&& functor) xrstl_noexcept
		{
			if (!handler<Functor>::empty_function(functor))
			{
				handler<Functor>::template create<Functor>(m_functor_storage, xrstl::forward<Functor>(functor));
		...
		fixed_function(fixed_function&& function) xrstl_noexcept
		{
		...
		fixed_function(const fixed_function& function) xrstl_noexcept
		{
			copy(function);
		}

		~fixed_function() xrstl_noexcept
		{
			if (m_manager)
			{
		...
		Result operator()(Args... args) const xrstl_noexcept
		{
			xrstl_assert(m_invoker != nullptr);
			return m_invoker(m_functor_storage, xrstl::forward<Args>(args)...);
		}

		explicit operator bool() const xrstl_noexcept
		{
	...
	private:
		...
		void copy(const fixed_function& f) xrstl_noexcept
		{
